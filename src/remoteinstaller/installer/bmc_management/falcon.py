...
class FALCON(BMC):
    ...
    def _set_setup_nfs(self, nfs_host, mount_path):
        ...
        # NFS server IP
        try:
            cmd = RAW_SET_RIS_NFS_IP % (self._convert_to_hex(nfs_host))
        ...
        try:
            ...
            time.sleep(2)
            self._run_ipmitool_raw_command(RAW_SET_RIS_NFS_PATH % (self._convert_to_hex(mount_path)))
            time.sleep(2)
            self._run_ipmitool_raw_command(RAW_SET_RIS_PROGRESS % '0x00')
    ...
    def _check_vmedia_mount_state(self, enabled):
        ...
        while tries > 0:
            ...
            if (status == '01') if enabled else (status == '00'):
    ...
    def _set_image_name(self, image_filename):
        try:
            ...
            self._run_ipmitool_raw_command(RAW_SET_IMG_NAME % self._convert_to_hex(image_filename))
        except Exception as err:
