...
class FALCON(BMC):
    ...
    def try_attach_virtual_cd(self, nfs_host, nfs_mount, boot_iso_filename):
        ...
        if not self._set_image_name(boot_iso_filename):
            log.warning('Failed to set image name. Raising exception')
        ...
        log.debug("Virtual Media setup succeeded for nfs://%s/%s/%s",
    ...
    def attach_virtual_cd(self, nfs_host, nfs_mount, boot_iso_filename):
        ...
        while tries > 0:
            try:
                ...
                tries -= 1
                log.warning("Failed with exception: '%s'. %d tries remaining.",
                            str(err), tries)
                if tries <= 0:
                    raise err
